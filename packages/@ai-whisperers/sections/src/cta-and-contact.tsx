...
export function CtaBanner({ pageContent }: SectionComponentProps) {
  ...
  return (
    <section className="py-24 text-center text-white bg-brand-gradient">
  ...
  return (
    <section className="py-24 bg-surface-alt">
      <div className="max-w-[800px] mx-auto text-center px-4">
        ...
        <a href={d.bookingUrl || d.ctaHref || 'https://wa.me/595982515138?text=Quiero%20agendar%20una%20consulta'}
          className="inline-block px-10 py-4 rounded-full font-bold text-base no-underline hover:opacity-90 bg-whatsapp text-white">{d.ctaText || d.ctaLabel || 'Agendar consulta gratuita'}</a>
...
export function ContactDetailsSection({ pageContent, data, locale }: SectionComponentProps) {
  const d = data || pageContent || {}
  if (!d.whatsapp && !d.email) return null

  const localeHoursKeys: Record<string, string[]> = {
    es: ['Lun–Vie', 'Sáb', 'Dom'],
    en: ['Mon–Fri', 'Sat', 'Sun'],
    nl: ['Ma–Vr', 'Za', 'Zo'],
    de: ['Mo–Fr', 'Sa', 'So'],
    ...
    <section className="py-24">
      <div className="max-w-[600px] mx-auto text-center px-4">
        ...
        <div className="flex flex-col gap-4">
          {d.whatsapp && <a href={`https://wa.me/${d.whatsapp.replace(/[^0-9]/g,'')}`} target="_blank" className="flex items-center justify-center gap-3 p-4 rounded-lg no-underline font-semibold text-white bg-whatsapp">
          ...
          {hours && <p className="text-text-muted text-xs"><span className="text-accent">◷</span> {hours}</p>}
        </div>
      </div>
    </section>
  )
