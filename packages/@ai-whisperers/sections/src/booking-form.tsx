...
export function BookingFormSection({ data, locale }: any) {
  const d = data || {}
  const lang = (locale || 'es') as string
        ...
        {step === 1 && (
          <div className="bg-white p-8 rounded-2xl shadow-sm">
            <div className="space-y-4">
              <input type="text" placeholder={t('namePlaceholder') || ({ es: 'Nombre completo', en: 'Full name', nl: 'Volledige naam', de: 'Vollständiger Name' } as any)[lang] || 'Nombre'} value={form.name} onChange={update('name')}
                className="w-full p-4 border border-border rounded-lg text-sm outline-none" />
              <input type="email" placeholder={t('emailPlaceholder') || ({ es: 'Email', en: 'Email', nl: 'E-mail', de: 'E-Mail' } as any)[lang] || 'Email'} value={form.email} onChange={update('email')}
                className="w-full p-4 border border-border rounded-lg text-sm outline-none" />
              <input type="tel" placeholder={t('phonePlaceholder') || ({ es: 'WhatsApp / Teléfono', en: 'WhatsApp / Phone', nl: 'WhatsApp / Telefoon', de: 'WhatsApp / Telefon' } as any)[lang] || 'WhatsApp'} value={form.phone} onChange={update('phone')}
                className="w-full p-4 border border-border rounded-lg text-sm outline-none" />
              <textarea placeholder={t('messagePlaceholder') || ({ es: 'Comentarios adicionales (opcional)', en: 'Additional comments (optional)', nl: 'Extra opmerkingen (optioneel)', de: 'Weitere Anmerkungen (optional)' } as any)[lang] || 'Mensaje'} value={form.message} onChange={update('message')} rows={3}
          ...
          <div className="bg-white p-8 rounded-2xl shadow-sm text-center">
            ...
            <div className="flex gap-3 justify-center">
              ...
              <a href={`https://wa.me/595982515138?text=${encodeURIComponent(`Hola! Quiero info sobre ${programs.find((p: any) => p.id === selectedProgram)?.title || ''}. Soy ${form.name}, email: ${form.email}, tel: ${form.phone}.${form.message ? ' ' + form.message : ''}`)}`}
                target="_blank"
                className="px-6 py-3 bg-whatsapp text-white rounded-full font-bold text-sm no-underline inline-block hover:opacity-90">
